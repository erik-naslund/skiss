...
describe('SPEC §8, the report', () => {
  test('every key the mapping does not carry is reported, once, on its element', () => {
    expect(
      ...
      { kind: 'slot_usage', element: 'Book.title' },
  ...
  test('schema-level boilerplate is not reported; anything else at that level is, once', () => {
    expect(dropped({ classes: {}, settings: {}, subsets: {} })).toEqual([
      { kind: 'schema', element: 'foreign', detail: '`settings`, `subsets`' },
    ]);
  });
