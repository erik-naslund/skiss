...
Or without installing: `npx skiss diagram model.skiss`.
...
`skiss compile model.skiss` writes a LinkML schema instead, as YAML on standard output. `--json` writes the same schema as JSON, `--name` sets the schema name (it defaults to the file name without its extension, and to `schema` when reading standard input), and `-o`, `-`, `--strict` and the diagnostics work exactly as they do for `diagram`.

`npm install skiss` also gives you the library: `compile`, `formatDiagnostic`, `parse`, `resolve`, `toMermaid` and `VERSION`, browser-safe.
